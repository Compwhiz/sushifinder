...
var bScreen = blessed.bScreen({
    autoPadding: true
});

bScreen.key('q', function() {
    process.exit(0);
});
...
var emailText = blessed.text({
    parent: emailForm,
    content: 'Select one of the following email service providers for {underline}contact' +
    ' form{/underline} and {underline}password reset{/underline}.',
    padding: 1,
    bg: 'red',
    fg: 'white',
    tags: true
...
home.on('select', function(child, index) {
    switch (index) {
        ...
        case 1:
            addClusterSupport();
            home.append(success);
            success.setContent('New file {underline}cluster_app.js{/underline} has been created.'+
            ' Your app is now able to use more than 1 CPU by running {underline}node cluster_app.js{/underline},'+
            ' which in turn spawns multiple instances of {underline}app.js{/underline}');
            success.focus();
            bScreen.render();
            break;
        default:
